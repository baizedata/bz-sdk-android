...
public class ChannelUtils {
    ...
    private static final String UTM_SOURCE_KEY = "BAIZE_UTM_SOURCE";
    private static final String UTM_MEDIUM_KEY = "BAIZE_UTM_MEDIUM";
    private static final String UTM_TERM_KEY = "BAIZE_UTM_TERM";
    private static final String UTM_CONTENT_KEY = "BAIZE_UTM_CONTENT";
    private static final String UTM_CAMPAIGN_KEY = "BAIZE_UTM_CAMPAIGN";
    private static final String SHARED_PREF_UTM_FILE = "baizedata.utm";
    ...
    public static String getDeviceInfo(Context mContext, String androidId) {
        return String.format("android_id=%s##imei=%s##imei_old=%s##imei_slot1=%s##imei_slot2=%s##imei_meid=%s##mac=%s##oaid=%s",
                androidId,
                BaizeUtils.getIMEI(mContext),
                BaizeUtils.getIMEIOld(mContext),
                BaizeUtils.getSlot(mContext, 0),
                BaizeUtils.getSlot(mContext, 1),
                BaizeUtils.getMEID(mContext),
                BaizeUtils.getMacAddress(mContext),
                BzDeviceUtils.getOAID(mContext));
    }
    ...
    public static void mergeUtmByMetaData(Context context, JSONObject properties) throws JSONException {
        ...
        for (Map.Entry<String, String> entry : UTM_MAP.entrySet()) {
            if (entry != null) {
                String utmValue = BaizeUtils.getApplicationMetaData(context, entry.getKey());
                ...
                if (hasUtmProperties) {
                    ...
                    for (String sourceKey : sChannelSourceKeySet) {
                        try {
                            ...
                            BzDataHelper.assertKey(sourceKey);
    ...
    public static void loadUtmByLocal(Context context) {
        try {
            SharedPreferences utmPref = BaizeUtils.getSharedPreferences(context);
            sLatestUtmProperties.clear();
