...
class BaizeActivityLifecycleCallbacks implements Application.ActivityLifecycleCallbacks {
    ...
    private final BaizeAPI mBaizeDataInstance;
    ...
    private Runnable timer = new Runnable() {
        @Override
        public void run() {
            if (mBaizeDataInstance.isAutoTrackEnabled() && isAutoTrackAppEnd()) {
                generateAppEndData();
            }
        }
    };

    BaizeActivityLifecycleCallbacks(BaizeAPI instance, PersistentFirstStart firstStart,
                                    PersistentFirstDay firstDay, Context context) {
        this.mBaizeDataInstance = instance;
        this.mFirstStart = firstStart;
        this.mFirstDay = firstDay;
        this.mContext = context;
        this.mDbAdapter = DbAdapter.getInstance();
        this.isMultiProcess = mBaizeDataInstance.isMultiProcess();
        this.sessionTime = mDbAdapter.getSessionIntervalTime();
        try {
            final PackageManager manager = mContext.getPackageManager();
            final PackageInfo info = manager.getPackageInfo(mContext.getPackageName(), 0);
            app_version = info.versionName;
            lib_version = BaizeAPI.VERSION;
        } catch (final Exception e) {
            BzLog.i(TAG, "Exception getting version name = ", e);
        }
        initHandler();
    }
    ...
    public void onActivityCreated(Activity activity, Bundle bundle) {
        BaizeUtils.handleSchemeUrl(activity, activity.getIntent());
    }
    ...
    public void onActivityStarted(Activity activity) {
        try {
            ...
            BaizeUtils.mergeJSONObject(activityProperty, endDataProperty);
            if (isMultiProcess) {
            ...
            if (startActivityCount == 1) {
                if (mBaizeDataInstance.isSaveDeepLinkInfo()) {// 保存 utm 信息时,在 endData 中合并保存的 latestUtm 信息。
                    BaizeUtils.mergeJSONObject(ChannelUtils.getLatestUtmProperties(), endDataProperty);
                }
                ...
                if (sessionTimeOut) {
                    ...
                    try {
                        mBaizeDataInstance.appBecomeActive();
                    } catch (Exception e) {
                    ...
                    if (resumeFromBackground) {
                        //先从缓存中读取 SDKConfig
                        mBaizeDataInstance.applySDKConfigFromCache();
                        mBaizeDataInstance.resumeTrackScreenOrientation();
//                    mBaizeDataInstance.resumeTrackTaskThread();
                    }
                    //每次启动 App，重新拉取最新的配置信息
                    mBaizeDataInstance.pullSDKConfigFromServer();

                    try {
                        if (mBaizeDataInstance.isAutoTrackEnabled() && !mBaizeDataInstance.isAutoTrackEventTypeIgnored(BaizeAPI.AutoTrackEventType.APP_START)) {
                            if (firstStart) {
                            ...
                            BaizeUtils.mergeJSONObject(activityProperty, properties);
                            if (ChannelUtils.parseUtmFromActivity(activity, true, mBaizeDataInstance.isSaveDeepLinkInfo())) {
                                BaizeUtils.mergeJSONObject(ChannelUtils.getUtmProperties(), properties);
                            }
                            mBaizeDataInstance.track("$AppStart", properties);
                        }
            ...
            if (startTimerCount++ == 0) {
                 ...
                 *  2. 多进程的情况下只会开启一个打点器；
                 */
                BaizeTimer.getInstance().timer(timer, 0, TIME_INTERVAL);
            }
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    ...
    public void onActivityResumed(final Activity activity) {
        try {
            ...
            if (ChannelUtils.parseUtmFromActivity(activity, false, mBaizeDataInstance.isSaveDeepLinkInfo())) {
                utmProperties = ChannelUtils.getUtmProperties();
                ChannelUtils.removeDeepLinkInfo(endDataProperty);
                BaizeUtils.mergeJSONObject(ChannelUtils.getLatestUtmProperties(), endDataProperty);
            }
            if (mBaizeDataInstance.isAutoTrackEnabled() && !mBaizeDataInstance.isActivityAutoTrackAppViewScreenIgnored(activity.getClass())
                    && !mBaizeDataInstance.isAutoTrackEventTypeIgnored(BaizeAPI.AutoTrackEventType.APP_VIEW_SCREEN)) {
                JSONObject properties = new JSONObject();
                if (utmProperties != null && utmProperties.length() > 0) {
                    BaizeUtils.mergeJSONObject(utmProperties, properties);
                }
                BaizeUtils.mergeJSONObject(activityProperty, properties);
                ...
                mBaizeDataInstance.trackViewScreen(BaizeUtils.getScreenUrl(activity), properties);
            }
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    ...
    public void onActivityStopped(Activity activity) {
        try {
            ...
            if (mBaizeDataInstance.isMultiProcess()) {
                startActivityCount = mDbAdapter.getActivityCount();
        ...
        try {
            if (mBaizeDataInstance.isAutoTrackEnabled() && isAutoTrackAppEnd()) {
                if (!TextUtils.isEmpty(jsonEndData)) {
                    ...
                    mBaizeDataInstance.track("$AppEnd", properties);
                    mDbAdapter.commitAppEndData(""); // 保存的信息只使用一次就置空，防止后面状态错乱再次发送。
                    mBaizeDataInstance.flushSync();
                }
            }
        } catch (Exception e) {
            BzLog.printStackTrace(e);
    ...
    private boolean isSessionTimeOut() {
        ...
        BzLog.d(TAG, "SessionTimeOut:" + sessionTimeOut);
    ...
    private void initHandler() {
        try {
            ...
            mContext.getContentResolver().registerContentObserver(DbParams.getInstance().getSessionTimeUri(),
                    false, new BaizeActivityStateObserver(handler));
        } catch (Exception ex) {
            BzLog.printStackTrace(ex);
        }
    }
    ...
    private void resetState() {
        try {
            mBaizeDataInstance.stopTrackScreenOrientation();
            mBaizeDataInstance.resetPullSDKConfigTimer();
            HeatMapService.getInstance().stop();
            VisualizedAutoTrackService.getInstance().stop();
            mBaizeDataInstance.appEnterBackground();
            resumeFromBackground = true;
            mBaizeDataInstance.clearLastScreenUrl();
//            mBaizeDataInstance.stopTrackTaskThread();
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    }
    ...
    private class BaizeActivityStateObserver extends ContentObserver {
         ...
         */
        BaizeActivityStateObserver(Handler handler) {
            super(handler);
        }
    ...
    private boolean isAutoTrackAppEnd() {
        return !mBaizeDataInstance.isAutoTrackEventTypeIgnored(BaizeAPI.AutoTrackEventType.APP_END);
    }
}