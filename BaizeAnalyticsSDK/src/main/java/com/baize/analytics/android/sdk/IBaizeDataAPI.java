...
public interface IBaizeDataAPI extends IFragmentAPI {
    /**
     * 返回预置属性
     *
     ...
     */
    void enableAutoTrack(List<BaizeAPI.AutoTrackEventType> eventTypeList);
     ...
     */
    void disableAutoTrack(List<BaizeAPI.AutoTrackEventType> eventTypeList);
     ...
     */
    void disableAutoTrack(BaizeAPI.AutoTrackEventType autoTrackEventType);
...
//    void showUpX5WebView(Object x5WebView);
     ...
     */
    @Deprecated
    void ignoreAutoTrackEventType(BaizeAPI.AutoTrackEventType autoTrackEventType);
     ...
     */
    @Deprecated
    void ignoreAutoTrackEventType(List<BaizeAPI.AutoTrackEventType> eventTypeList);
     ...
     */
    boolean isAutoTrackEventTypeIgnored(BaizeAPI.AutoTrackEventType eventType);

    /**
     * 判断某个 AutoTrackEventType 是否被忽略
     *
     * @param autoTrackEventType BaizeAnalyticsAutoTrackEventType 中的事件类型，可通过 '|' 进行连接传递
     * @return true 被忽略; false 没有被忽略
     */
...
//    void enableHeatMap();
    ...
    /**
     * 获取当前用户的匿名 ID
     * 若调用前未调用 {@link #identify(String)} 设置用户的匿名 ID，SDK 会优先调用 {@link BaizeUtils#getAndroidID(Context)}获取 Android ID，
...
//     * 这是 Baize 进阶功能，请参考文档 https://sensorsdata.cn/manual/track_installation.html
//     *
...
//    void trackChannelEvent(String eventName, JSONObject properties);
    ...
    /**
     * 以阻塞形式将所有本地缓存的日志发送到 Baize，该方法不能在 UI 线程调用。
     */
    void flushSync();
     ...
     */
    void registerDynamicSuperProperties(BaizeDynamicSuperProperties dynamicSuperProperties);
     ...
     */
    void setTrackEventCallBack(BaizeTrackEventCallBack trackEventCallBack);
