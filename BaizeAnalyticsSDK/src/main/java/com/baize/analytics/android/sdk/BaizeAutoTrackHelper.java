...
import com.baize.analytics.android.sdk.util.WindowHelper;
...
public class BaizeAutoTrackHelper {
    private static final String TAG = "BaizeAutoTrackHelper";
    ...
    private static void traverseView(String fragmentName, ViewGroup root) {
        try {
            ...
            for (int i = 0; i < childCount; ++i) {
                ...
                child.setTag(R.id.baize_tag_view_fragment_name, fragmentName);
    ...
    public static void onFragmentViewCreated(Object object, View rootView, Bundle bundle) {
        try {
            ...
            rootView.setTag(R.id.baize_tag_view_fragment_name, fragmentName);
    ...
    public static void trackRN(Object target, int reactTag, int s, boolean b) {
        try {
            if (!BaizeAPI.sharedInstance().isReactNativeAutoTrackEnabled()) {
                return;
            }

            //关闭 AutoTrack
            if (!BaizeAPI.sharedInstance().isAutoTrackEnabled()) {
                return;
            }

            //$AppClick 被过滤
            if (BaizeAPI.sharedInstance().isAutoTrackEventTypeIgnored(BaizeAPI.AutoTrackEventType.APP_CLICK)) {
                return;
            }
            ...
            if (target != null) {
                ...
                if (resolveViewMethod != null) {
                    ...
                    if (object != null) {
                        ...
                        //$screen_name & $title
                        if (activity != null) {
                            BaizeUtils.mergeJSONObject(AopUtil.buildTitleAndScreenName(activity), properties);
            ...
            BaizeAPI.sharedInstance().track(AopConstants.APP_CLICK_EVENT_NAME, properties);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    }
    ...
    private static void trackFragmentAppViewScreen(Object fragment) {
        try {
            if (BaizeAPI.sharedInstance().isAutoTrackEventTypeIgnored(BaizeAPI.AutoTrackEventType.APP_VIEW_SCREEN)) {
                return;
            }

            if (!BaizeAPI.sharedInstance().isTrackFragmentAppViewScreenEnabled()) {
                return;
            }
            ...
            boolean isAutoTrackFragment = BaizeAPI.sharedInstance().isFragmentAutoTrackAppViewScreen(fragment.getClass());
            ...
            BaizeAPI.sharedInstance().trackViewScreen(BaizeUtils.getScreenUrl(fragment), properties);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    }
    ...
    public static void trackExpandableListViewOnGroupClick(ExpandableListView expandableListView, View view,
        ...
        try {
            ...
            //Activity 被忽略
            if (activity != null) {
                if (BaizeAPI.sharedInstance().isActivityAutoTrackAppClickIgnored(activity.getClass())) {
                    return;
                }
            }
            ...
            // fragment 忽略
            if (fragment != null) {
                if (BaizeAPI.sharedInstance().isActivityAutoTrackAppClickIgnored(fragment.getClass())) {
                    return;
                }
            }
            ...
            // 获取 View 自定义属性
            JSONObject p = (JSONObject) view.getTag(R.id.baize_tag_view_properties);
            if (p != null) {
            ...
            if (listAdapter != null) {
                if (listAdapter instanceof BaizeExpandableListViewItemTrackProperties) {
                    try {
                        BaizeExpandableListViewItemTrackProperties trackProperties = (BaizeExpandableListViewItemTrackProperties) listAdapter;
    ...
    public static void trackExpandableListViewOnChildClick(ExpandableListView expandableListView, View view,
        ...
        try {
            ...
            //获取 View 自定义属性
            JSONObject properties = (JSONObject) view.getTag(R.id.baize_tag_view_properties);
    ...
    public static void trackTabLayoutSelected(Object object, Object tab) {
        try {
            ...
            if (isFragment) {
                if (BaizeAPI.sharedInstance().isActivityAutoTrackAppClickIgnored(object.getClass())) {
                    return;
                }
            }
        ...
        try {
            ...
            try {
                if (dialog.getWindow() != null) {
                    String idString = (String) dialog.getWindow().getDecorView().getTag(R.id.baize_tag_view_id);
                    if (!TextUtils.isEmpty(idString)) {
                        properties.put(AopConstants.ELEMENT_ID, idString);
                    }
                }
    ...
    public static void trackListView(AdapterView<?> adapterView, View view, int position) {
        try {
            ...
            if (adapter instanceof BaizeAdapterViewItemTrackProperties) {
                try {
                    BaizeAdapterViewItemTrackProperties objectProperties = (BaizeAdapterViewItemTrackProperties) adapter;
    ...
    public static void trackDrawerOpened(View view) {
        try {
            ...
            BaizeAPI.sharedInstance().setViewProperties(view, jsonObject);
    ...
    public static void trackViewOnClick(View view, boolean isFromUser) {
        try {
            ...
            if (BaizeUtils.isDoubleClick(view)) {
                return;
            }
    ...
    public static void track(String eventName, String properties) {
        try {
            ...
            BaizeAPI.sharedInstance().track(eventName, pro);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    }
        ...
        try {
            ...
            if (uri != null) {
                ...
                if ("heatmap".equals(host)) {
                    ...
                    String featureCode = uri.getQueryParameter("feature_code");
                    String postUrl = uri.getQueryParameter("url");
                    String serverUrl = BaizeAPI.sharedInstance().getServerUrl();
    ...
    private static void showPopupWindowDialog(Activity activity, Uri uri) {
        try {
            Class<?> clazz = Class.forName("com.baize.sf.ui.utils.PreviewUtil");
    ...
    private static void showDebugModeSelectDialog(final Activity activity, final String infoId) {
        try {
            DebugModeSelectDialog dialog = new DebugModeSelectDialog(activity, BaizeAPI.sharedInstance().getDebugMode());
            ...
            dialog.setOnDebugModeDialogClickListener(new DebugModeSelectDialog.OnDebugModeViewClickListener() {
                ...
                @Override
                public void setDebugMode(Dialog dialog, BaizeAPI.DebugMode debugMode) {
                    BaizeAPI.sharedInstance().setDebugMode(debugMode);
                    dialog.cancel();
                }
            });
            dialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
                ...
                public void onCancel(DialogInterface dialog) {
                    ...
                    String serverUrl = BaizeAPI.sharedInstance().getServerUrl();
                    BaizeAPI.DebugMode mCurrentDebugMode = BaizeAPI.sharedInstance().getDebugMode();
                    if (BaizeAPI.sharedInstance().isNetworkRequestEnable() && !TextUtils.isEmpty(serverUrl) && !TextUtils.isEmpty(infoId) && mCurrentDebugMode != BaizeAPI.DebugMode.DEBUG_OFF) {
                        new SendDebugIdThread(serverUrl, BaizeAPI.sharedInstance().getDistinctId(), infoId, ThreadNameConstants.THREAD_SEND_DISTINCT_ID).start();
                    }
                    String currentDebugToastMsg = "";
                    if (mCurrentDebugMode == BaizeAPI.DebugMode.DEBUG_OFF) {
                        currentDebugToastMsg = "已关闭调试模式，请重新扫描二维码进行开启";
                    } else if (mCurrentDebugMode == BaizeAPI.DebugMode.DEBUG_ONLY) {
                        currentDebugToastMsg = "开启调试模式，校验数据，但不进行数据导入；关闭 App 进程后，将自动关闭调试模式";
                    } else if (mCurrentDebugMode == BaizeAPI.DebugMode.DEBUG_AND_TRACK) {
                        currentDebugToastMsg = "开启调试模式，校验数据，并将数据导入到白泽分析中；关闭 App 进程后，将自动关闭调试模式";
                    }
                    Toast.makeText(activity, currentDebugToastMsg, Toast.LENGTH_LONG).show();
                    BzLog.info(TAG, "您当前的调试模式是：" + mCurrentDebugMode, null);
                }
            });
            dialog.show();
        } catch (Exception e) {
...
//        try {
//            if (!BaizeAPI.sharedInstance().isNetworkRequestEnable()) {
//                showDialog(context, "已关闭网络请求（NetworkRequest），无法使用 App 点击分析，请开启后再试！");
//                return;
//            }
//            if (!BaizeAPI.sharedInstance().isAppHeatMapConfirmDialogEnabled()) {
...
//                return;
//            }
//            if (!BaizeAPI.sharedInstance().isVisualizedAutoTrackEnabled()) {
//                showDialog(context, "SDK 没有被正确集成，请联系贵方技术人员开启可视化全埋点。");
//                return;
//            }
//            if (!BaizeAPI.sharedInstance().isVisualizedAutoTrackConfirmDialogEnabled()) {
    ...
    private static class SendDebugIdThread extends Thread {
        ...
        private void sendHttpRequest(String serverUrl, boolean isRedirects) {
            ...
            try {
                ...
                BzLog.info(TAG, String.format("DebugMode URL:%s", url), null);
                connection = (HttpURLConnection) url.openConnection();
                if (connection == null) {
                    BzLog.info(TAG, String.format("can not connect %s,shouldn't happen", url.toString()), null);
                    return;
                }
                SSLSocketFactory sf = BaizeAPI.sharedInstance().getSSLSocketFactory();
                ...
                writer.flush();
                BzLog.info(TAG, String.format("DebugMode request body : %s", requestBody), null);
                ...
                BzLog.info(TAG, String.format(Locale.CHINA, "DebugMode 后端的响应码是:%d", responseCode), null);
                if (!isRedirects && BaizeHttpURLConnectionHelper.needRedirects(responseCode)) {
                    String location = BaizeHttpURLConnectionHelper.getLocation(connection, serverUrl);
