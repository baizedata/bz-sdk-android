...
import com.baize.analytics.android.sdk.data.persistent.PersistentFirstDay;
import com.baize.analytics.android.sdk.data.persistent.PersistentFirstTrackInstallation;
import com.baize.analytics.android.sdk.exceptions.InvalidDataException;
import com.baize.analytics.android.sdk.internal.FragmentAPI;
import com.baize.analytics.android.sdk.internal.IFragmentAPI;
import com.baize.analytics.android.sdk.listener.BzEventListener;
import com.baize.analytics.android.sdk.util.AopUtil;
import com.baize.analytics.android.sdk.util.BaizeUtils;
import com.baize.analytics.android.sdk.util.Base64Coder;
import com.baize.analytics.android.sdk.util.BzDataHelper;
import com.baize.analytics.android.sdk.util.ChannelUtils;
import com.baize.analytics.android.sdk.util.DateFormatUtils;
...
import com.baize.analytics.android.sdk.data.PersistentLoader;
import com.baize.analytics.android.sdk.data.persistent.PersistentDistinctId;
import com.baize.analytics.android.sdk.data.persistent.PersistentFirstStart;
import com.baize.analytics.android.sdk.data.persistent.PersistentFirstTrackInstallationWithCallback;
import com.baize.analytics.android.sdk.data.persistent.PersistentRemoteSDKConfig;
import com.baize.analytics.android.sdk.data.persistent.PersistentSuperProperties;
 ...
 * Baize SDK
 */
public class BaizeAPI implements IBaizeDataAPI {
    ...
    // Maps each token to a singleton BaizeAPI instance
    private static final Map<Context, BaizeAPI> sInstanceMap = new HashMap<>();
    private static final String TAG = "SA.BaizeAPI";
    static boolean mIsMainProcess = false;
    static boolean SHOW_DEBUG_INFO_VIEW = true;
    private static BaizeSDKRemoteConfig mSDKRemoteConfig;
    private static BaizeGPSLocation mGPSLocation;
    /* 远程配置 */
    private static BzConfigOptions mBzConfigOptions;
    ...
    private BaizeScreenOrientationDetector mOrientationDetector;
    private BaizeDynamicSuperProperties mDynamicSuperProperties;
    private SimpleDateFormat mIsFirstDayDateFormat;
    private SSLSocketFactory mSSLSocketFactory;
    private BaizeTrackEventCallBack mTrackEventCallBack;
    private List<BzEventListener> mEventListenerList;
    ...
    BaizeAPI(Context context, String serverURL, DebugMode debugMode) {
        mContext = context;
        ...
        mAndroidId = BaizeUtils.getAndroidID(mContext);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            ...
            final BaizeActivityLifecycleCallbacks lifecycleCallbacks =
                    new BaizeActivityLifecycleCallbacks(this, mFirstStart, mFirstDay, context);
            app.registerActivityLifecycleCallbacks(lifecycleCallbacks);
            app.registerActivityLifecycleCallbacks(AppStateManager.getInstance());
        }

        BzLog.i(TAG, String.format(Locale.CHINA, "Initialized the instance of Baize SDK with server"
                + " url '%s', flush interval %d ms, debugMode: %s", mServerUrl, mBzConfigOptions.mFlushInterval, debugMode));
     ...
     * @return BaizeAPI 单例
     */
    public static BaizeAPI sharedInstance(Context context) {
        if (isSDKDisabled()) {
            return new BaizeAPIEmptyImplementation();
        }
        ...
        synchronized (sInstanceMap) {
            ...
            BaizeAPI instance = sInstanceMap.get(appContext);

            if (null == instance) {
                BzLog.i(TAG, "The static method sharedInstance(context, serverURL, debugMode) should be called before calling sharedInstance()");
                return new BaizeAPIEmptyImplementation();
            }
            return instance;
        }
     ...
     * {@link BaizeAPI.DebugMode}
     * @return BaizeAPI 单例
     */
    @Deprecated
    public static BaizeAPI sharedInstance(Context context, String serverURL, DebugMode debugMode) {
     ...
     */
    @Deprecated
    public static BaizeAPI sharedInstance(Context context, String serverURL) {
     ...
     *
     * @param context App 的 Context
     * @param bzConfigOptions SDK 的配置项
     * @return BaizeAPI 单例
     */
    @Deprecated
    public static BaizeAPI sharedInstance(Context context, BzConfigOptions bzConfigOptions) {
        mBzConfigOptions = bzConfigOptions;
        BaizeAPI baizeAPI = getInstance(context, bzConfigOptions.mServerUrl, DebugMode.DEBUG_OFF);
        if (!baizeAPI.mSDKConfigInit) {
            baizeAPI.applySAConfigOptions();
        }
        return baizeAPI;
    }
     ...
     * @param bzConfigOptions SDK 的配置项
     */
    public static void startWithConfigOptions(Context context, BzConfigOptions bzConfigOptions) {
        if (context == null || bzConfigOptions == null) {
            throw new NullPointerException("Context、BzConfigOptions 不可以为 null");
        }
        mBzConfigOptions = bzConfigOptions;
        BaizeAPI baizeAPI = getInstance(context, bzConfigOptions.mServerUrl, DebugMode.DEBUG_OFF);
        if (!baizeAPI.mSDKConfigInit) {
    ...
    private static BaizeAPI getInstance(Context context, String serverURL, DebugMode debugMode) {
        if (null == context) {
            return new BaizeAPIEmptyImplementation();
        }
        ...
        synchronized (sInstanceMap) {
            ...
            BaizeAPI instance = sInstanceMap.get(appContext);
            if (null == instance) {
                instance = new BaizeAPI(appContext, serverURL, debugMode);
    ...
    public static BaizeAPI sharedInstance() {
        if (isSDKDisabled()) {
            return new BaizeAPIEmptyImplementation();
        }
        ...
        synchronized (sInstanceMap) {
            if (sInstanceMap.size() > 0) {
                Iterator<BaizeAPI> iterator = sInstanceMap.values().iterator();
                if (iterator.hasNext()) {
    ...
    static void allInstances(InstanceProcessor processor) {
        synchronized (sInstanceMap) {
            for (final BaizeAPI instance : sInstanceMap.values()) {
    ...
    private Map<String, Object> setupDeviceInfo() {
        ...
        deviceInfo.put("$lib_code", BuildConfig.VERSION_CODE);
        deviceInfo.put("$os", "Android");
        deviceInfo.put("$os_version", Build.VERSION.RELEASE == null ? "UNKNOWN" : Build.VERSION.RELEASE);
        deviceInfo.put("$manufacturer", BaizeUtils.getManufacturer());
        ...
        try {
            ...
            deviceInfo.put("$screen_width", BaizeUtils.getNaturalWidth(rotation, screenWidth, screenHeight));
            deviceInfo.put("$screen_height", BaizeUtils.getNaturalHeight(rotation, screenWidth, screenHeight));
        } catch (Exception e) {
        ...
        String carrier = BaizeUtils.getCarrier(mContext);
        ...
        Integer zone_offset = BaizeUtils.getZoneOffset();
        if (zone_offset != null) {
    ...
    /**
     * 更新 BaizeSDKRemoteConfig
     *
     * @param sdkRemoteConfig BaizeSDKRemoteConfig 在线控制 SDK 的配置
     * @param effectImmediately 是否立即生效
     */
    private void setSDKRemoteConfig(BaizeSDKRemoteConfig sdkRemoteConfig, boolean effectImmediately) {
        try {
            if (sdkRemoteConfig.isDisableSDK()) {
                BaizeSDKRemoteConfig cachedConfig = BaizeUtils.toSDKRemoteConfig(mPersistentRemoteSDKConfig.get());
                if (!cachedConfig.isDisableSDK()) {
                    track("DisableBaizeSDK");
                }
            }
    ...
    void pullSDKConfigFromServer() {
        ...
        if (mBzConfigOptions != null && !mBzConfigOptions.mDisableRandomTimeRequestRemoteConfig
                && !BaizeUtils.isRequestValid(mContext, mBzConfigOptions.mMinRequestInterval, mBzConfigOptions.mMaxRequestInterval)) {
            return;
        }
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            if (mBzConfigOptions != null && !TextUtils.isEmpty(mBzConfigOptions.mRemoteConfigUrl)) {
                                configUrl = mBzConfigOptions.mRemoteConfigUrl;
                            } else {
                            ...
                            if (TextUtils.isEmpty(configUrl)) {
                                BzLog.i(TAG, "Remote config url is null or empty.");
                                return;
                            }
                            ...
                            if (!TextUtils.isEmpty(configUrl)) {
                                ...
                                BzLog.d(TAG, "Android remote config url:" + configUrl);
                            }
                            ...
                            if (responseCode == 200) {
                                ...
                                if (!TextUtils.isEmpty(data)) {
                                    BaizeSDKRemoteConfig sdkRemoteConfig = BaizeUtils.toSDKRemoteConfig(data);
    ...
    void applySDKConfigFromCache() {
        try {
            BaizeSDKRemoteConfig sdkRemoteConfig = BaizeUtils.toSDKRemoteConfig(mPersistentRemoteSDKConfig.get());

            if (sdkRemoteConfig == null) {
                sdkRemoteConfig = new BaizeSDKRemoteConfig();
            }
    ...
    public JSONObject getPresetProperties() {
        ...
        try {
            ...
            properties.put("$lib_code", BuildConfig.VERSION_CODE);
    ...
    public void enableLog(boolean enable) {
        BzLog.setEnableLog(enable);
    }

    @Override
    public long getMaxCacheSize() {
        return mBzConfigOptions.mMaxCacheSize;
    }

    @Override
    public void setMaxCacheSize(long maxCacheSize) {
        mBzConfigOptions.setMaxCacheSize(maxCacheSize);
    }

    @Override
    public void setFlushNetworkPolicy(int networkType) {
        mBzConfigOptions.setNetworkTypePolicy(networkType);
    }

    int getFlushNetworkPolicy() {
        return mBzConfigOptions.mNetworkTypePolicy;
    }

    @Override
    public int getFlushInterval() {
        return mBzConfigOptions.mFlushInterval;
    }

    @Override
    public void setFlushInterval(int flushInterval) {
        mBzConfigOptions.setFlushInterval(flushInterval);
    }

    @Override
    public int getFlushBulkSize() {
        return mBzConfigOptions.mFlushBulkSize;
    }

    @Override
    public void setFlushBulkSize(int flushBulkSize) {
        if (flushBulkSize < 0) {
            BzLog.i(TAG, "The value of flushBulkSize is invalid");
        }
        mBzConfigOptions.setFlushBulkSize(flushBulkSize);
    }
    ...
    public void setSessionIntervalTime(int sessionIntervalTime) {
        ...
        if (sessionIntervalTime < 10 * 1000 || sessionIntervalTime > 5 * 60 * 1000) {
            BzLog.i(TAG, "SessionIntervalTime:" + sessionIntervalTime + " is invalid, session interval time is between 10s and 300s.");
            return;
        }
    ...
    public void setGPSLocation(double latitude, double longitude) {
        try {
            if (mGPSLocation == null) {
                mGPSLocation = new BaizeGPSLocation();
            }
        ...
        try {
            if (enable) {
                if (mOrientationDetector == null) {
                    mOrientationDetector = new BaizeScreenOrientationDetector(mContext, SensorManager.SENSOR_DELAY_NORMAL);
                }
    ...
    public void setCookie(String cookie, boolean encode) {
        try {
            if (encode) {
                this.mCookie = URLEncoder.encode(cookie, Base64Coder.CHARSET_UTF8);
            } else {
                this.mCookie = cookie;
            }
        } catch (Exception e) {
    ...
    public void enableAutoTrack(List<AutoTrackEventType> eventTypeList) {
        try {
            ...
            for (AutoTrackEventType autoTrackEventType : eventTypeList) {
                mBzConfigOptions.setAutoTrackEventType(mBzConfigOptions.mAutoTrackEventType | autoTrackEventType.eventValue);
            }
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    ...
    private void enableAutoTrack(int autoTrackEventType) {
        try {
            ...
            this.mAutoTrack = true;
            mBzConfigOptions.setAutoTrackEventType(mBzConfigOptions.mAutoTrackEventType | autoTrackEventType);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    }
    ...
    public void trackAppCrash() {
        BaizeExceptionHandler.enableAppCrash();
    }
    ...
    public void enableReactNativeAutoTrack() {
        mBzConfigOptions.enableReactNativeAutoTrack(true);
    }

    @Override
    public boolean isReactNativeAutoTrackEnabled() {
        return mBzConfigOptions.mRNAutoTrackEnabled;
    }
...
//            BzLog.d(TAG, "For applications targeted to API level JELLY_BEAN or below, this feature NOT SUPPORTED");
//            return;
//        }
//
//        if (webView != null) {
...
//
    ...
    public boolean isActivityAutoTrackAppViewScreenIgnored(Class<?> activity) {
        ...
        if (activity.getAnnotation(BaizeIgnoreTrackAppViewScreenAndAppClick.class) != null) {
            return true;
        }

        return activity.getAnnotation(BaizeIgnoreTrackAppViewScreen.class) != null;
    ...
    public boolean isActivityAutoTrackAppClickIgnored(Class<?> activity) {
        ...
        return activity.getAnnotation(BaizeIgnoreTrackAppClick.class) != null;
    ...
    public void ignoreAutoTrackEventType(AutoTrackEventType autoTrackEventType) {
        ...
        if (mBzConfigOptions.mAutoTrackEventType == 0) {
            return;
        }

        int union = mBzConfigOptions.mAutoTrackEventType | autoTrackEventType.eventValue;
        if (union == autoTrackEventType.eventValue) {
            mBzConfigOptions.setAutoTrackEventType(0);
        } else {
            mBzConfigOptions.setAutoTrackEventType(autoTrackEventType.eventValue ^ union);
        }
    ...
    public void ignoreAutoTrackEventType(List<AutoTrackEventType> eventTypeList) {
        ...
        for (AutoTrackEventType autoTrackEventType : eventTypeList) {
            if ((mBzConfigOptions.mAutoTrackEventType | autoTrackEventType.eventValue) == mBzConfigOptions.mAutoTrackEventType) {
                mBzConfigOptions.setAutoTrackEventType(mBzConfigOptions.mAutoTrackEventType ^ autoTrackEventType.eventValue);
            }
        }
    ...
    public boolean isAutoTrackEventTypeIgnored(int autoTrackEventType) {
        if (mSDKRemoteConfig != null) {
            if (mSDKRemoteConfig.getAutoTrackMode() != BaizeSDKRemoteConfig.REMOTE_EVENT_TYPE_NO_USE) {
                if (mSDKRemoteConfig.getAutoTrackMode() == 0) {
                    return true;
                }
        ...
        return (mBzConfigOptions.mAutoTrackEventType | autoTrackEventType) != mBzConfigOptions.mAutoTrackEventType;
    }

    @Override
    public void setViewID(View view, String viewID) {
        if (view != null && !TextUtils.isEmpty(viewID)) {
            view.setTag(R.id.baize_tag_view_id, viewID);
        }
    }
        ...
        try {
            if (view != null && !TextUtils.isEmpty(viewID)) {
                if (view.getWindow() != null) {
                    view.getWindow().getDecorView().setTag(R.id.baize_tag_view_id, viewID);
                }
            }
        } catch (Exception e) {
            BzLog.printStackTrace(e);
    ...
    public void setViewID(Object alertDialog, String viewID) {
        try {
            ...
            if (!TextUtils.isEmpty(viewID)) {
                ...
                if (window != null) {
                    window.getDecorView().setTag(R.id.baize_tag_view_id, viewID);
                }
            }
        } catch (Exception e) {
            BzLog.printStackTrace(e);
    ...
    public void setViewActivity(View view, Activity activity) {
        try {
            ...
            view.setTag(R.id.baize_tag_view_activity, activity);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    }
    ...
    public void setViewFragmentName(View view, String fragmentName) {
        try {
            ...
            view.setTag(R.id.baize_tag_view_fragment_name2, fragmentName);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
        }
    }
    ...
    public void ignoreView(View view) {
        if (view != null) {
            view.setTag(R.id.baize_tag_view_ignored, "1");
        }
    }
    ...
    public void ignoreView(View view, boolean ignore) {
        if (view != null) {
            view.setTag(R.id.baize_tag_view_ignored, ignore ? "1" : "0");
        }
    }
    ...
    public void setViewProperties(View view, JSONObject properties) {
        ...
        view.setTag(R.id.baize_tag_view_properties, properties);
    }
...
//        return mBzConfigOptions.mVisualizedEnabled;
//    }
//
//    boolean isVisualizedAutoTrackConfirmDialogEnabled() {
//        return mBzConfigOptions.mVisualizedConfirmDialogEnabled;
//    }
//
//    @Override
//    public void enableVisualizedAutoTrackConfirmDialog(boolean enable) {
//        mBzConfigOptions.enableVisualizedAutoTrackConfirmDialog(enable);
//    }
//
//    @Override
//    public void enableVisualizedAutoTrack() {
//        mBzConfigOptions.enableVisualizedAutoTrack(true);
//    }
//
//    @Override
...
//        return mBzConfigOptions.mHeatMapEnabled;
//    }
...
//        return mBzConfigOptions.mHeatMapConfirmDialogEnabled;
//    }

//    @Override
//    public void enableAppHeatMapConfirmDialog(boolean enable) {
//        mBzConfigOptions.enableHeatMapConfirmDialog(enable);
//    }
//
//    @Override
//    public void enableHeatMap() {
//        mBzConfigOptions.enableHeatMap(true);
//    }
    ...
    public void resetAnonymousId() {
        synchronized (mDistinctId) {
            if (BaizeUtils.isValidAndroidId(mAndroidId)) {
            ...
            if (mEventListenerList != null) {
                for (BzEventListener eventListener : mEventListenerList) {
    ...
    public void identify(final String distinctId) {
        try {
            BzDataHelper.assertValue(distinctId);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
            return;
        }
    ...
    public void login(final String loginId, final JSONObject properties) {
        try {
            BzDataHelper.assertValue(loginId);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
            return;
        }
...
//                        BaizeUtils.mergeJSONObject(_properties, profileProperties);
...
//                try {
//                    try {
//                        _properties.put("$is_channel_callback_event", BaizeUtils.isFirstChannelEvent(mContext, eventName));
//                        if (!ChannelUtils.hasUtmProperties(_properties)) {
//                            ChannelUtils.mergeUtmByMetaData(mContext, _properties);
//                        }
//                        if (!ChannelUtils.hasUtmProperties(_properties)) {
...
//    }
    ...
    public void trackTimer(final String eventName, final TimeUnit timeUnit) {
        ...
        mTrackTaskManager.addTrackEventTask(new Runnable() {
            ...
            public void run() {
                try {
                    BzDataHelper.assertKey(eventName);
            ...
            public void run() {
                try {
                    if (!TextUtils.isEmpty(url) || properties != null) {
                        ...
                        mLastScreenUrl = url;
                        if (properties != null) {
                            BaizeUtils.mergeJSONObject(properties, trackProperties);
                        }
        ...
        mTrackTaskManager.addTrackEventTask(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    trackViewScreen(BaizeUtils.getScreenUrl(activity), properties);
                } catch (Exception e) {
                    BzLog.printStackTrace(e);
                }
            }
        ...
        mTrackTaskManager.addTrackEventTask(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (fragment.getClass().isAnnotationPresent(BaizeFragmentTitle.class)) {
                        BaizeFragmentTitle baizeFragmentTitle = fragment.getClass().getAnnotation(BaizeFragmentTitle.class);
                        if (baizeFragmentTitle != null) {
                            title = baizeFragmentTitle.title();
                        }
                    }
                    ...
                    if (Build.VERSION.SDK_INT >= 11) {
                        ...
                        if (activity != null) {
                            if (TextUtils.isEmpty(title)) {
                                title = BaizeUtils.getActivityTitle(activity);
                            }
                    ...
                    if (fragment instanceof ScreenAutoTracker) {
                        ...
                        if (otherProperties != null) {
                            BaizeUtils.mergeJSONObject(otherProperties, properties);
                        }
                    }
                    trackViewScreen(BaizeUtils.getScreenUrl(fragment), properties);
                } catch (Exception e) {
                    BzLog.printStackTrace(e);
                }
            }
    ...
    void appEnterBackground() {
        synchronized (mTrackTimer) {
            try {
                ...
                BzLog.i(TAG, "appEnterBackground error:" + e.getMessage());
            }
        }
    }
    ...
    void appBecomeActive() {
        synchronized (mTrackTimer) {
            try {
                ...
                BzLog.i(TAG, "appBecomeActive error:" + e.getMessage());
            }
        }
    }
    ...
    @Override
    public void registerDynamicSuperProperties(BaizeDynamicSuperProperties dynamicSuperProperties) {
        mDynamicSuperProperties = dynamicSuperProperties;
    }

    @Override
    public void setTrackEventCallBack(BaizeTrackEventCallBack trackEventCallBack) {
    ...
    public void stopTrackThread() {
        if (mTrackTaskManagerThread != null && !mTrackTaskManagerThread.isStopped()) {
            ...
            BzLog.i(TAG, "Data collection thread has been stopped");
        }
    }
    ...
    public void startTrackThread() {
        if (mTrackTaskManagerThread == null || mTrackTaskManagerThread.isStopped()) {
            ...
            BzLog.i(TAG, "Data collection thread has been started");
        }
    }
    ...
    public void registerSuperProperties(JSONObject superProperties) {
        try {
            ...
            BzDataHelper.assertPropertyTypes(superProperties);
            synchronized (mSuperProperties) {
                JSONObject properties = mSuperProperties.get();
                BaizeUtils.mergeSuperJSONObject(superProperties, properties);
...
//    }
    ...
    public void setServerUrl(String serverUrl) {
        try {
            ...
            if (!TextUtils.isEmpty(hostServer) && hostServer.contains("_")) {
                BzLog.i(TAG, "Server url " + serverUrl + " contains '_' is not recommend，" +
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mDebugMode == DebugMode.DEBUG_ONLY) {
                        info = "现在您打开了 Baize SDK 的 'DEBUG_ONLY' 模式，此模式下只校验数据但不导入数据，数据出错时会以 Toast 的方式提示开发者，请上线前一定使用 DEBUG_OFF 模式。";
                    } else if (mDebugMode == DebugMode.DEBUG_AND_TRACK) {
                        info = "现在您打开了Baize SDK 的 'DEBUG_AND_TRACK' 模式，此模式下校验数据并且导入数据，数据出错时会以 Toast 的方式提示开发者，请上线前一定使用 DEBUG_OFF 模式。";
                    }
                    CharSequence appName = BaizeUtils.getAppName(mContext);
    ...
    public void trackEventFromH5(String eventInfo) {
        try {
            ...
            if (eventType == EventType.TRACK_SIGNUP) {
                ...
                eventObject.put("$is_login_id", true);
            } else {
                eventObject.put(distinctIdKey, getAnonymousId());
                eventObject.put("$is_login_id", false);
            }
            eventObject.put("$app_id", this.mAppId);
            ...
            if (eventType.isTrack()) {
                ...
                synchronized (mSuperProperties) {
                    JSONObject superProperties = mSuperProperties.get();
                    BaizeUtils.mergeJSONObject(superProperties, propertiesObject);
                }
                ...
                try {
                    if (mDynamicSuperProperties != null) {
                        ...
                        if (dynamicSuperProperties != null) {
                            BaizeUtils.mergeJSONObject(dynamicSuperProperties, propertiesObject);
                        }
                    }
                } catch (Exception e) {
                    BzLog.printStackTrace(e);
                ...
                BaizeUtils.mergeJSONObject(ChannelUtils.getLatestUtmProperties(), propertiesObject);
            }
            ...
            if (eventType.isTrack()) {
                ...
                if (!enterDb) {
                    BzLog.d(TAG, eventName + " event can not enter database");
                    return;
                }
            }
    ...
    private boolean isEnterDb(String eventName, JSONObject eventProperties) {
        ...
        if (mTrackEventCallBack != null) {
            BzLog.d(TAG, "SDK have set trackEvent callBack");
            try {
                ...
                try {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (!(value instanceof CharSequence || value instanceof Number || value
                            ...
                            BzLog.d(TAG, String.format("The property value must be an instance of " +
                        ...
                        if ("app_crashed_reason".equals(key)) {
                            if (value instanceof String && ((String) value).length() > 8191 * 2) {
                                BzLog.d(TAG, "The property value is too long. [key='" + key
    ...
    private void trackEvent(final EventType eventType, String eventName, final JSONObject properties, final String
        ...
        try {
            ...
            BzDataHelper.assertPropertyTypes(properties);
            ...
            try {
                ...
                if (eventType.isTrack()) {
                    ...
                    if (!"$AppEnd".equals(eventName)) {
                        //合并 $latest_utm 属性
                        BaizeUtils.mergeJSONObject(ChannelUtils.getLatestUtmProperties(), sendProperties);
                    }

                    synchronized (mSuperProperties) {
                        JSONObject superProperties = mSuperProperties.get();
                        BaizeUtils.mergeJSONObject(superProperties, sendProperties);
                    }
                    ...
                    try {
                        if (mDynamicSuperProperties != null) {
                            JSONObject dynamicSuperProperties = mDynamicSuperProperties.getDynamicSuperProperties();
                            if (dynamicSuperProperties != null) {
                                BaizeUtils.mergeJSONObject(dynamicSuperProperties, sendProperties);
                            }
                        }
                    } catch (Exception e) {
                        BzLog.printStackTrace(e);
                ...
                if (null != properties) {
                    ...
                    BaizeUtils.mergeJSONObject(properties, sendProperties);
                }
                ...
                libProperties.put("$lib_code", BuildConfig.VERSION_CODE);
                ...
                if (TextUtils.isEmpty(getLoginId())) {
                    dataObj.put("$is_login_id", false);
                } else {
                    dataObj.put("$is_login_id", true);
                }
                dataObj.put("$app_id", this.mAppId);
    ...
    boolean isMultiProcess() {
        return mBzConfigOptions.mEnableMultiProcess;
    }
    ...
    private void trackItemEvent(String itemType, String itemId, String eventType, JSONObject properties) {
        try {
            BzDataHelper.assertKey(itemType);
            BzDataHelper.assertValue(itemId);
            BzDataHelper.assertPropertyTypes(properties);
    ...
    private void initSAConfig(String serverURL, String packageName) {
        ...
        if (mBzConfigOptions == null) {
            this.mSDKConfigInit = false;
            mBzConfigOptions = new BzConfigOptions(serverURL);
        } else {
            this.mSDKConfigInit = true;
        }

        if (mBzConfigOptions.mInvokeLog) {
            enableLog(mBzConfigOptions.mLogEnabled);
        } else {
            enableLog(configBundle.getBoolean("com.baize.analytics.android.EnableLogging",
                    this.mDebugMode != DebugMode.DEBUG_OFF));
        }

        setServerUrl(serverURL);

        if (mBzConfigOptions.mEnableTrackAppCrash) {
            BaizeExceptionHandler.enableAppCrash();
        }

        if (mBzConfigOptions.mFlushInterval == 0) {
            mBzConfigOptions.setFlushInterval(configBundle.getInt("com.baize.analytics.android.FlushInterval",
                    5000));
        }

        if (mBzConfigOptions.mFlushBulkSize == 0) {
            mBzConfigOptions.setFlushBulkSize(configBundle.getInt("com.baize.analytics.android.FlushBulkSize",
                    100));
        }

        if (mBzConfigOptions.mMaxCacheSize == 0) {
            mBzConfigOptions.setMaxCacheSize(32 * 1024 * 1024L);
        }

        this.mAutoTrack = configBundle.getBoolean("com.baize.analytics.android.AutoTrack",
                false);
        if (mBzConfigOptions.mAutoTrackEventType != 0) {
            enableAutoTrack(mBzConfigOptions.mAutoTrackEventType);
            this.mAutoTrack = true;
        }

        if (!mBzConfigOptions.mInvokeHeatMapEnabled) {
            mBzConfigOptions.mHeatMapEnabled = configBundle.getBoolean("com.baize.analytics.android.HeatMap",
                    false);
        }

        if (!mBzConfigOptions.mInvokeHeatMapConfirmDialog) {
            mBzConfigOptions.mHeatMapConfirmDialogEnabled = configBundle.getBoolean("com.baize.analytics.android.EnableHeatMapConfirmDialog",
                    true);
        }

        if (!mBzConfigOptions.mInvokeVisualizedEnabled) {
            mBzConfigOptions.mVisualizedEnabled = configBundle.getBoolean("com.baize.analytics.android.VisualizedAutoTrack",
                    false);
        }

        if (!mBzConfigOptions.mInvokeVisualizedConfirmDialog) {
            mBzConfigOptions.mVisualizedConfirmDialogEnabled = configBundle.getBoolean("com.baize.analytics.android.EnableVisualizedAutoTrackConfirmDialog",
                    true);
        }

        enableTrackScreenOrientation(mBzConfigOptions.mTrackScreenOrientationEnabled);

        if (!TextUtils.isEmpty(mBzConfigOptions.mAnonymousId)) {
            identify(mBzConfigOptions.mAnonymousId);
        }

        SHOW_DEBUG_INFO_VIEW = configBundle.getBoolean("com.baize.analytics.android.ShowDebugInfoView",
                true);

        this.mDisableDefaultRemoteConfig = configBundle.getBoolean("com.baize.analytics.android.DisableDefaultRemoteConfig",
                false);

        this.mMainProcessName = BaizeUtils.getMainProcessName(mContext);
        if (TextUtils.isEmpty(this.mMainProcessName)) {
            this.mMainProcessName = configBundle.getString("com.baize.analytics.android.MainProcessName");
        }
        mIsMainProcess = BaizeUtils.isMainProcess(mContext, mMainProcessName);

        this.mDisableTrackDeviceId = configBundle.getBoolean("com.baize.analytics.android.DisableTrackDeviceId",
                false);
...
//            public void run() {
//                try {
//                    BzDataHelper.assertKey(pushTypeKey);
//                    if (TextUtils.isEmpty(pushId)) {
//                        BzLog.d(TAG, "pushId is empty");
//                        return;
//                    }
//                    String distinctId = getDistinctId();
//                    String distinctPushId = distinctId + pushId;
//                    SharedPreferences sp = BaizeUtils.getSharedPreferences(mContext);
...
//    }
    ...
    public void addEventListener(BzEventListener eventListener) {
        try {
    ...
    boolean isSaveDeepLinkInfo() {
        return mBzConfigOptions.mEnableSaveDeepLinkInfo;
    }

    /**
     * Debug 模式，用于检验数据导入是否正确。该模式下，事件会逐条实时发送到 Baize，并根据返回值检查
     * 数据导入是否正确。
     * Debug 模式有三种：
     * DEBUG_OFF - 关闭DEBUG模式
     * DEBUG_ONLY - 打开DEBUG模式，但该模式下发送的数据仅用于调试，不进行数据导入
     * DEBUG_AND_TRACK - 打开DEBUG模式，并将数据导入到Baize中
     */
