...
class VisualizedAutoTrackViewCrawler implements VTrack {
    ...
    VisualizedAutoTrackViewCrawler(Activity activity, String resourcePackageName, String featureCode, String postUrl) {
        ...
        try {
            mPostUrl = URLDecoder.decode(postUrl, Base64Coder.CHARSET_UTF8);
    ...
    private class ViewCrawlerHandler extends Handler {
        ...
        private void sendSnapshot(JSONObject message) {
            ...
            try {
                ...
                if (null == mSnapshot) {
                    BzLog.i(TAG, "Snapshot should be initialize at first calling.");
                    return;
                }
                ...
                BzLog.i(TAG, "Payload with snapshot config required with snapshot request", e);
                return;
            } catch (final EditProtocol.BadInstructionsException e) {
                BzLog.i(TAG, "VisualizedAutoTrack server sent malformed message with snapshot request", e);
                return;
            }
            ...
            try {
                ...
                BzLog.i(TAG, "Can't write snapshot request to server", e);
            } finally {
                try {
                    writer.close();
                } catch (final IOException e) {
                    BzLog.i(TAG, "Can't close writer.", e);
                }
            }
            postSnapshot(out);
        }
