...
/* package */ class AppWebViewInterface {
    ...
    public String sensorsdata_call_app() {
        try {
            ...
            String loginId = BaizeAPI.sharedInstance(mContext).getLoginId();
            if (!TextUtils.isEmpty(loginId)) {
                properties.put("distinct_id", loginId);
                properties.put("is_login", true);
            } else {
                properties.put("distinct_id", BaizeAPI.sharedInstance(mContext).getAnonymousId());
    ...
    public void sensorsdata_track(String event) {
        try {
            BaizeAPI.sharedInstance(mContext).trackEventFromH5(event, enableVerify);
        } catch (Exception e) {
    ...
    public boolean sensorsdata_verify(String event) {
        try {
            ...
            return BaizeAPI.sharedInstance(mContext)._trackEventFromH5(event);
        } catch (Exception e) {
            BzLog.printStackTrace(e);
            return false;
        }
