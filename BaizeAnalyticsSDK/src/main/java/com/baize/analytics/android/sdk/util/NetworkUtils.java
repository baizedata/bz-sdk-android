...
public class NetworkUtils {
    ...
    public static String networkType(Context context) {
        try {
            // 检测权限
            if (!BaizeUtils.checkHasPermission(context, Manifest.permission.ACCESS_NETWORK_STATE)) {
                return "NULL";
            }
    ...
    private static int toNetworkType(String networkType) {
        if ("NULL".equals(networkType)) {
            return BaizeAPI.NetworkType.TYPE_ALL;
        } else if ("WIFI".equals(networkType)) {
            return BaizeAPI.NetworkType.TYPE_WIFI;
        } else if ("2G".equals(networkType)) {
            return BaizeAPI.NetworkType.TYPE_2G;
        } else if ("3G".equals(networkType)) {
            return BaizeAPI.NetworkType.TYPE_3G;
        } else if ("4G".equals(networkType)) {
            return BaizeAPI.NetworkType.TYPE_4G;
        } else if ("5G".equals(networkType)) {
            return BaizeAPI.NetworkType.TYPE_5G;
        }
        return BaizeAPI.NetworkType.TYPE_ALL;
    }
